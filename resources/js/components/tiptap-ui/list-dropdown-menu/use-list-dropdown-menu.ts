...
export function useListDropdownMenu(config?: UseListDropdownMenuConfig) {
  ...
  const listInSchema = useMemo(
    () => types.some((type) => isNodeInSchema(type, editor)),
    [editor, types]
  )
  ...
  useEffect(() => {
    ...
    const sync = () => {
      const nextCanToggleAny = canToggleAnyList(editor, types)
      const nextIsAnyActive = isAnyListActive(editor, types)
      const nextActiveType = getActiveListType(editor, types)

      setCanToggleAny(nextCanToggleAny)
      setIsAnyActive(nextIsAnyActive)
